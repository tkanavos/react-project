...
router.get('/signIn', async (request, response, next) => {
    console.log("ok")
    try{
        const results = await client.query('SELECT * FROM countries WHERE id = $1');
...
router.post('/', async (request, response, next) => {
    console.log("ok")
        ...
        response.status(204).send('A new country has been added to the database')
        }catch(error){
            if (error) {
                throw error
            }
...
router.put('/:id', async (request, response, next) => {
    console.log("ok")
        ...
        response.status(206).send('Country has been updated in the database')
        }catch(error){
            if (error) {
                throw error
            }
...
router.delete('/', async (request, response, next) => {
    ...
    try{
        ...
        response.status(208).send(`Country deleted with ID: ${id}`)
    }catch(error){
        if (error) {
            throw error
        }
